...
#ifdef _MSC_VER
#pragma warning(push)
// warning C4251: using non-dll-exported type in dll-exported type, firing on
// std::vector<frame_ptr> and others for some reason 4275 is the same thing but
// for base classes
...
// Some type sufficient for an instruction pointer, currently always an alias to
// std::uintptr_t
...
// This is used over std::optional because the library is C++11 and also
// std::optional is a bit heavy-duty for this use.
template<typename T,
...
struct CPPTRACE_EXPORT stacktrace_frame {
    ...
    bool operator==(const stacktrace_frame& other) const
    {
        return raw_address == other.raw_address &&
               object_address == other.object_address && line == other.line &&
               column == other.column && filename == other.filename &&
               symbol == other.symbol;
    }
...
private:
    void print(std::ostream& stream, bool color, bool newline_at_end,
               const char* header) const;
    void print_with_snippets(std::ostream& stream, bool color,
                             bool newline_at_end, const char* header) const;
    friend void print_terminate_trace();
};

CPPTRACE_EXPORT raw_trace generate_raw_trace(std::size_t skip = 0);
CPPTRACE_EXPORT raw_trace generate_raw_trace(std::size_t skip,
                                             std::size_t max_depth);
CPPTRACE_EXPORT object_trace generate_object_trace(std::size_t skip = 0);
CPPTRACE_EXPORT object_trace generate_object_trace(std::size_t skip,
                                                   std::size_t max_depth);
CPPTRACE_EXPORT stacktrace generate_trace(std::size_t skip = 0);
CPPTRACE_EXPORT stacktrace generate_trace(std::size_t skip,
                                          std::size_t max_depth);

// Path max isn't so simple, so I'm choosing 4096 which seems to encompass what
// all major OS's expect and should be fine in all reasonable cases.
...
// signal-safe
CPPTRACE_EXPORT std::size_t safe_generate_raw_trace(frame_ptr* buffer,
                                                    std::size_t size,
                                                    std::size_t skip = 0);
// signal-safe
CPPTRACE_EXPORT std::size_t safe_generate_raw_trace(frame_ptr* buffer,
...
// signal-safe
CPPTRACE_EXPORT void get_safe_object_frame(frame_ptr address,
                                           safe_object_frame* out);
CPPTRACE_EXPORT bool can_signal_safe_unwind();

// utilities:
CPPTRACE_EXPORT std::string demangle(const std::string& name);
CPPTRACE_EXPORT std::string get_snippet(const std::string& path,
                                        std::size_t line,
...
// tracing exceptions:
namespace detail {
// This is a helper utility, if the library weren't C++11 an std::variant would
// be used
...
public:
    ...
    explicit lazy_trace_holder(stacktrace&& _resolved_trace)
        : resolved(true),
          resolved_trace(std::move(_resolved_trace))
    {
    }
    // logistics
...
CPPTRACE_EXPORT raw_trace
get_raw_trace_and_absorb(std::size_t skip, std::size_t max_depth) noexcept;
CPPTRACE_EXPORT raw_trace
get_raw_trace_and_absorb(std::size_t skip = 0) noexcept;
CPPTRACE_EXPORT std::string exception_type_name();
} // namespace detail
...
// I hate to have to expose anything about implementation detail but the idea
// here is that
...
public:
    explicit exception_with_message(
        ...
        raw_trace&& trace = detail::get_raw_trace_and_absorb()) noexcept
        : lazy_exception(std::move(trace)),
          user_message(std::move(message_arg))
    {
    }
...
[[noreturn]] CPPTRACE_EXPORT void
rethrow_and_wrap_if_needed(std::size_t skip = 0);
} // namespace cpptrace

#if defined(CPPTRACE_STD_FORMAT) && defined(__cpp_lib_format)
template<>
struct std::formatter<cpptrace::stacktrace_frame>
